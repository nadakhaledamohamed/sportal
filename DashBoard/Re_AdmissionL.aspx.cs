...
public partial class DashBoard_Re_AdmissionL : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataAdapter adr;
    ...
    protected void btnSubmit_Click1(object sender, EventArgs e)
    {
        ...
        if (Page.IsValid)
        {
            ...
            using (con = new SqlConnection(DashBoard))
            {
                using (cmd = new SqlCommand("Add_ReAdmisson_Requestless2S", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@Requestid", Requestid);
                    cmd.Parameters.AddWithValue("@Petition", txtReason.Text);
